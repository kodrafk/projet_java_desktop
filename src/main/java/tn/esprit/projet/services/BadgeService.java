...
public class BadgeService {
    ...
    public int calculateStreak(int userId) {
        try (PreparedStatement ps = tn.esprit.projet.utils.MyBDConnexion.getInstance().getCnx().prepareStatement(
    ...
    private boolean hasComeback(int userId) {
        try (PreparedStatement ps = tn.esprit.projet.utils.MyBDConnexion.getInstance().getCnx().prepareStatement(
            ...
            if (rs.next() && rs.getTimestamp("last_log") != null) {
                ...
                try (PreparedStatement ps2 = tn.esprit.projet.utils.MyBDConnexion.getInstance().getCnx().prepareStatement(
