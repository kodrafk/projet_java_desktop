...
public class ComplaintResponseService implements CRUD<ComplaintResponse> {
    ...
    public void supprimer(int id) {
        try (PreparedStatement ps = cnx.prepareStatement("DELETE FROM complaint_response WHERE id=?")) {
            ps.setInt(1, id);
            ps.executeUpdate();
        } catch (SQLException ex) { ex.printStackTrace(); }
    }
    ...
    public ComplaintResponse getById(int id) {
        try (PreparedStatement ps = cnx.prepareStatement("SELECT * FROM complaint_response WHERE id=?")) {
            ps.setInt(1, id);
    ...
    public List<ComplaintResponse> getAll() {
        ...
        try (Statement st = cnx.createStatement();
             ResultSet rs = st.executeQuery("SELECT * FROM complaint_response ORDER BY id DESC")) {
    ...
    public void supprimerByComplaintId(int complaintId) {
        try (PreparedStatement ps = cnx.prepareStatement("DELETE FROM complaint_response WHERE complaint_id=?")) {
            ps.setInt(1, complaintId);
            ps.executeUpdate();
        } catch (SQLException ex) { ex.printStackTrace(); }
    }
    ...
    private ComplaintResponse mapResultSet(ResultSet rs) throws SQLException {
        ...
        if (ts != null) r.setResponseDate(ts.toLocalDateTime());
        return r;
    }
}
