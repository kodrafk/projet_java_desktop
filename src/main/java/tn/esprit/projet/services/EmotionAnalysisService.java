...
public class EmotionAnalysisService {
    ...
    private static EmotionAnalysis parseEmotionResponse(String jsonResponse) {
        try {
            ...
            return new EmotionAnalysis(emotion, score, urgency, getRecommendation(emotion, urgency, score));
        } catch (Exception e) {
    ...
    private static String getRecommendation(String emotion, int urgency, double score) {
        if (urgency >= 4) return "🔴 URGENT - Assign to senior support staff immediately";
        else if (urgency == 3) return "🟠 HIGH - Prioritize for response within 2 hours";
        else if (urgency == 2) return "🟡 MEDIUM - Standard response within 24 hours";
        else return "🟢 LOW - Can be handled in regular queue";
    }

    public static String getUrgencyColor(int urgencyLevel) {
        switch (urgencyLevel) {
            case 5: case 4: return "#EF4444";
