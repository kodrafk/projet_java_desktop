...
public class DatabaseConnection {
     ...
     * Delegates to MyBDConnexion and validates the connection is still open.
     */
    public Connection getConnection() {
        try {
            ...
            if (cnx == null || cnx.isClosed() || !cnx.isValid(2)) {
                System.out.println("[DB] Connection stale — reconnecting...");
                MyBDConnexion.resetInstance();
                cnx = MyBDConnexion.getInstance().getCnx();
            }
            return cnx;
        } catch (Exception e) {
            System.err.println("[DB] getConnection error: " + e.getMessage());
