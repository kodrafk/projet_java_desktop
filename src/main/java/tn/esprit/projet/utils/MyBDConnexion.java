        ...
        try (java.sql.Statement st = cnx.createStatement()) {
            ...
            try (java.sql.PreparedStatement ps = cnx.prepareStatement(
                ...
                if (rs.next() && rs.getInt(1) == 0) {
                    ...
                    System.out.println("[initComplaintsTable] Added image_path column.");
                }
            }

            // Add incident_date column if it doesn't exist (for existing databases)
            try (java.sql.PreparedStatement ps = cnx.prepareStatement(
                    "SELECT COUNT(*) FROM information_schema.COLUMNS " +
                ...
                if (rs.next() && rs.getInt(1) == 0) {
                    ...
                    System.out.println("[initComplaintsTable] Added incident_date column.");
                }
            }
