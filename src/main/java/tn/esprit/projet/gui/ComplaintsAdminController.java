...
public class ComplaintsAdminController {
    ...
    private javafx.scene.layout.VBox createComplaintCard(Complaint c) {
        ...
        statusLbl.setStyle("-fx-background-color: " + statusColor + "20; -fx-text-fill: " + statusColor +
                           "; -fx-padding: 3 8; -fx-background-radius: 12; -fx-font-size: 11px; -fx-font-weight: bold;");
        ...
        if (!imagePaths.isEmpty()) {
            ...
            if (!imagesBox.getChildren().isEmpty()) card.getChildren().add(imagesBox);
        }
    ...
    private void showComplaintDetails(Complaint c) {
        ...
        lblPhone.setText(c.getPhoneNumber() != null ? c.getPhoneNumber() : "-");
    ...
    private void handleSave() {
        ...
        if (email != null && !email.isEmpty() && email.contains("@")) {
            ...
            tn.esprit.projet.services.EmailService.sendEmailAsync(email, subject, message);
        }

        Toast.show((javafx.stage.Stage) cardsContainer.getScene().getWindow(), "Response saved!", Toast.Type.SUCCESS);
        refreshData();
    }
    ...
    private void handleSuggestResponseAI() {
        ...
        ).thenAccept(suggestion -> javafx.application.Platform.runLater(() -> {
            fldResponse.setDisable(false);
            if (suggestion != null && !suggestion.isEmpty()) {
                fldResponse.setText(suggestion);
                Toast.show((javafx.stage.Stage) cardsContainer.getScene().getWindow(), "AI response generated!", Toast.Type.SUCCESS);
            } else {
                Toast.show((javafx.stage.Stage) cardsContainer.getScene().getWindow(), "AI quota limit reached. Try again tomorrow.", Toast.Type.ERROR);
            }
        })).exceptionally(ex -> {
    ...
    private void handleExportExcel() {
        ...
        fc.setInitialFileName("Complaints_Export.xlsx");
    ...
    private void handleExportPDF() {
        ...
        fc.setInitialFileName("Complaints_Report.pdf");
        File file = fc.showSaveDialog(cardsContainer.getScene().getWindow());
        if (file != null) {
            try {
                ...
                for (String h : new String[]{"ID", "User", "Title", "Date", "Rate", "Status"}) {
    ...
    private void filterData(String keyword) {
        ...
        if (keyword != null && !keyword.isEmpty()) {
            ...
            stream = stream.filter(c -> c.getTitle().toLowerCase().contains(lc) || c.getStatus().toLowerCase().contains(lc));
        }
        if (sort != null) {
            switch (sort) {
