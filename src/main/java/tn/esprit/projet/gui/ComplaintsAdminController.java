...
public class ComplaintsAdminController {
    ...
    private void handleViewImage() {
        ...
        java.util.List<String> imagePaths = selectedComplaint.getImagePathsList();
        if (imagePaths.isEmpty()) {
            Toast.show((javafx.stage.Stage) cardsContainer.getScene().getWindow(), "No image attached to this complaint.", Toast.Type.ERROR);
            return;
        }
        for (String path : imagePaths) {
            java.io.File file = new java.io.File(path);
            if (file.exists()) {
                ...
                root.setStyle("-fx-background-color: #1E293B; -fx-padding: 10;");
                ...
                Toast.show((javafx.stage.Stage) cardsContainer.getScene().getWindow(), "Image file not found: " + path, Toast.Type.ERROR);
            }
        }
    }
